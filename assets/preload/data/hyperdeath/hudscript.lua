...
function onCreate()
    ...
    function onStepHit()
        ...
        elseif curStep == 1855 then
            doTweenAlpha('vignetteMAD', 'vignette2', 1, 0.3, 'linear')
        elseif curStep == 3167 then
            doTweenAlpha('vignette1CALM', 'vignette', 0, 3, 'linear')
            doTweenAlpha('vignette2CALM', 'vignette2', 0, 3, 'linear')
        elseif curStep == 3711 then
            triggerEvent('Flash Camera White', 0.4)
    ...
    if curStep == 2112 then
        styleRating = 'ENRAGED'
        combo = combo + 1
        makeLuaText('styleRating' .. ratingCount, 'ENRAGED', 400, 905, 250 + upScrollOffset)
        ...
        setTextBorder('styleRating' .. ratingCount, 1, 'ff0000')
        setTextColor('styleRating' .. ratingCount, 'ff0000')
    ...
    if curStep >= 2110 and curStep <= 2655 then
    ...
    function onUpdate()
        ...
        if curStep <= 319 or (curStep >= 1855 and curStep <= 1919) or (curStep >= 2111 and curStep <= 2655) then
            setProperty('defaultCamZoom', 1.2)
        end

        if curStep == 3423 then
            doTweenZoom('finalZoom', 'camGame', 0.7, 10, 'sineInOut')
            doTweenX('finalFollowX', 'camFollow', 740, 10, 'sineInOut')
            doTweenY('finalFollowY', 'camFollow', 350, 10,'sineInOut')
            doTweenX('finalPosX', 'camFollowPos', 740, 10, 'sineInOut')
            doTweenY('finalPosY', 'camFollowPos', 350, 10,'sineInOut')
            doTweenAlpha('finaleLight', 'finaleBlend', 0.1, 10, 'linear')
        end
    ...
    if combo <= 9 then
        ...
        triggerEvent('Toggle Trail', '', '')
        afterImages = false
    end

    
    if rankVar >= 5 and afterImages ~= true then
        triggerEvent('Toggle Trail', '31B0D1', '31B0D1')
        afterImages = true
    end
    ...
    if n == 'Flash Camera White' then
        doTweenColor('bfcolor', 'boyfriendGroup', 'ffffff', 0.01, 'linear')
        doTweenColor('dadcolor', 'dadGroup', 'ffffff', 0.01, 'linear')
        setProperty('heresy.alpha', 1)
        setProperty('heresyRed.alpha', 0)
        setProperty('vignette2.alpha', 0)
    end
    ...
    function onSectionHit()
        ...
        if curSection == 53 then
            allowZoom = true
        elseif curSection == 60 then
            allowZoom = false
        end

        if mustHitSection == false and hudLeft == true and curStep < 3423 then
            if allowZoom == false then
            setProperty('defaultCamZoom', 0.85)
            end
        
            doTweenX('hudRight', 'camHUD', 35, 0.5, 'sineOut')
            doTweenY('hudUp', 'camHUD', 15, 0.5, 'sineOut')
            hudLeft = false
        elseif mustHitSection == true and hudLeft == false and curStep < 3423 then
            setProperty('defaultCamZoom', 1.2)
        
            doTweenX('hudLeft', 'camHUD', -30, 0.6, 'sineOut')
            doTweenY('hudDown', 'camHUD', -15, 0.6, 'sineOut')
            hudLeft = true
        end
