function onCreate()
    makeLuaSprite('sanctum2', 'bg_sanctum', -825,-500)
...
function onUpdate()
    ...
    if curStep > 160 then
        doTweenAlpha('introAlpha', 'introBlack', 0, 3, 'linear')
    end

    if curStep == 1776 then
        doTweenAlpha('PicobotDeath', 'introBlack', 1, 0.1, 'linear')
    end
    if curStep == 1888 then
        doTweenAlpha('JUDGEMENT', 'introBlack', 0, 0.1, 'linear')
    end
...
function textScroll()
    ...
    if curStep >= 10 then
        ...
        local wordSize4 = string.len('CURRENT OBJECTIVE:   SURVIVE')
            splitWord4 = {}   
        for i=check4, check4 do            
            splitWord4[i] = string.sub('CURRENT OBJECTIVE:   SURVIVE', i, i)
            check4= check4 +1;
