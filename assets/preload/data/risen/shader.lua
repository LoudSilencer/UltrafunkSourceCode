...
local shaderName4 = "morph"
local shaderName5 = "aura"
function onCreate()
...
function onUpdate(elapsed)
    setShaderFloat("thingA", "iTime" , os.clock())
...
if not lowQuality then
    if n == 'addBinary' then
        ...
        makeLuaSprite('thingA', 'bg_aura', -1300,-875)

        addLuaSprite('thingA', false)
        setScrollFactor('thingA', 0.9, 0.9)
        setProperty('thingA.alpha', 0)

        makeLuaSprite('heresy', 'bg_sanctum', -700,-575)
        ...
        setSpriteShader("thingA", "morph")
        setSpriteShader("dad", "aura")
        setProperty('thingA.alpha', 1)

        setProperty('thingA.alpha', 1)
        setProperty('heresy.alpha', .5)
    end 
