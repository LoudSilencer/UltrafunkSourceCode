...
function onCreate()
    ...
    makeLuaSprite('speedrunHud', '', 0, 0);
    makeGraphic('speedrunHud',275,300,'000000')
    addLuaSprite('speedrunHud', true);
    setObjectCamera('speedrunHud', 'other')
    setProperty('speedrunHud.alpha', 0.5)
    setProperty('speedrunHud.x', 30)
    setProperty('speedrunHud.y', 30)

    makeLuaSprite('speedrunLine', '', 0, 0);
    makeGraphic('speedrunLine',265,5,'ffffff')
    addLuaSprite('speedrunLine', true);
    setObjectCamera('speedrunLine', 'other')
    setProperty('speedrunLine.x', 35)
    setProperty('speedrunLine.y', 80)

    makeLuaText('speedrunTxt', 'Sub Five', 300, 95, 40)
    setTextSize('speedrunTxt', 30)
    setTextBorder('speedrunTxt', 0, '000000')
    setTextAlignment('speedrunTxt', 'left')
    setObjectCamera('speedrunTxt', 'other')
    addLuaText('speedrunTxt', false)
    ...
    makeLuaSprite('fist', 'fist', 0, 0);
    addLuaSprite('fist', true);
    setObjectCamera('fist', 'hud')
    scaleObject('fist', 0.60, 0.60)
    setProperty('fist.angle', -7)
    setObjectOrder('fist', getObjectOrder('strumLineNotes') -1)
    setProperty('fist.x', 365)
    setProperty('fist.y', 545)
    ...
    makeLuaSprite('gunHud', 'gunHud', 0, 0)
    addLuaSprite('gunHud', true);
    setObjectCamera('gunHud', 'hud')
    scaleObject('gunHud', 0.29,0.29)
    setProperty('gunHud.angle', -7)
    setProperty('gunHud.x', 105)
    setProperty('gunHud.y', 376)
	...
	setPropertyFromClass('GameOverSubstate', 'characterName', '');
	setPropertyFromClass('GameOverSubstate', 'deathSoundName', 'HEH1');
	setPropertyFromClass('GameOverSubstate', 'loopSoundName', 'HEH1');
	setPropertyFromClass('GameOverSubstate', 'endSoundName', 'HEH1');
    ...
    if disallowDisrespect == true then
        setProperty('fist.alpha', 0.2)
    else
        doTweenAlpha('fistAlpha', 'fist', 0.9, 0.3, 'linear')
    end
    ...
    if keyboardJustPressed('SPACE') and disallowDisrespect == false and not inGameOver then
        ...
        bonkChance = getRandomInt(1,10)
        if bonkChance == 10 then
            playSound('bonk', 0.6, 'bonk')
        else
            playSound('punch', 0.5, 'punch')
        end
        cameraShake('game', 0.01, 0.15)
        cameraShake('hud', 0.01, 0.15)
    ...
    function onSectionHit()
        ...
        if mustHitSection == false and curStep < 656 then
            setProperty('defaultCamZoom', 1.1)
        elseif mustHitSection == true and curStep < 656 then
